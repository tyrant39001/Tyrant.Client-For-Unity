...
public static class ToolbarExtender
{
	...
	static ToolbarExtender()
	{
		...
		FieldInfo toolIcons = m_toolbarType.GetField(fieldName,
	...
	static void OnGUI()
	{
		...
		DrawRect(leftRect, LeftToolbarGUI);
		DrawRect(rightRect, RightToolbarGUI);

		void DrawRect(Rect rect, Action action)
        {
			if (rect.width > 0)
			{
				GUILayout.BeginArea(rect);
				InvokeToolbarAction(action);
				GUILayout.EndArea();
			}
		}
	}
	...
	{
		InvokeToolbarAction(LeftToolbarGUI);
	}

	public static void OnToolbarGUIRight()
	{
		InvokeToolbarAction(RightToolbarGUI);
	}

	private static void InvokeToolbarAction(Action action)
    {
		if (action == null)
			return;
