...
public class AdvancedDropdownGUI
{
    private static class Styles
    {
        static Styles()
        {
            var svcType = typeof(Editor).Assembly.GetType("UnityEditor.StyleSheets.SVC`1");
            var svcColorType = svcType.MakeGenericType(typeof(Color));
            var svcColorInstance = Activator.CreateInstance(svcColorType, new object[2] { "--theme-toolbar-background-color", Color.black });
            var op_ImplicitMethod = svcColorType.GetMethod("op_Implicit", BindingFlags.Public | BindingFlags.Static);
            searchBackgroundColor = (Color)op_ImplicitMethod.Invoke(null, new object[1] { svcColorInstance });
        }
    ...
    {
        var content = GUIContentTemp(name, icon);
    ...
    internal protected virtual void DrawHeader(AdvancedDropdownItem group, Action backButtonPressed, bool hasParent)
    {
        var content = GUIContentTemp(group.name, group.icon);
    ...
    private static MethodInfo editorGUI_ToolbarSearchFieldMethod = typeof(EditorGUI).GetMethod("ToolbarSearchField", BindingFlags.Static | BindingFlags.NonPublic, null, new Type[3] { typeof(Rect), typeof(string), typeof(bool) }, null);
    internal virtual string DrawSearchFieldControl(string searchString)
    {
        ...
        var newSearch = /*EditorGUI.ToolbarSearchField(controlRect, searchString, false)*/(string)editorGUI_ToolbarSearchFieldMethod.Invoke(null, new object[3] { controlRect, searchString, false });
    ...
    internal Vector2 CalculateContentSize(AdvancedDropdownDataSource dataSource)
    {
        ...
        {
            var content = GUIContentTemp(child.name, child.icon);
    ...
    private GUIContent GUIContentTemp(string t, Texture i) => new GUIContent(t, i);
}

public static class AdvancedDropdownItemExtention
{
    private static MethodInfo advancedDropdownItem_IsSeparatorInfo = typeof(AdvancedDropdownItem).GetMethod("IsSeparator", BindingFlags.NonPublic | BindingFlags.Instance);

    public static bool IsSeparator(this AdvancedDropdownItem advancedDropdownItem)
    {
        return (bool)advancedDropdownItem_IsSeparatorInfo.Invoke(advancedDropdownItem, null);
    }
}