...
{
    private static MethodInfo containerWindow_fitRectToScreenMethod;
    static AdvancedDropdownWindow()
    {
        var containerWindowType = typeof(Editor).Assembly.GetType("UnityEditor.ContainerWindow");
        containerWindow_fitRectToScreenMethod = containerWindowType.GetMethod("FitRectToScreen", BindingFlags.NonPublic | BindingFlags.Static, null, new Type[3] { typeof(Rect), typeof(bool), typeof(bool) }, null);
    }
    ...
    protected virtual Vector2 CalculateWindowSize(Rect buttonRect)
    {
        ...
        var fitRect = (Rect)containerWindow_fitRectToScreenMethod.Invoke(null, new object[3] { new Rect(buttonRect.x, buttonRect.y, size.x, size.y), true, true });
    ...
    protected virtual void GoToParent()
    {
    ...
    {
        GoToChild(m_State.GetSelectedChild(m_CurrentlyRenderedTree));
    }

    protected void GoToChild(AdvancedDropdownItem advancedDropdownItem)
    {
        m_ViewsStack.Push(m_CurrentlyRenderedTree);
        m_LastTime = DateTime.Now.Ticks;
        if (m_NewAnimTarget < 0)
            ...
            m_NewAnimTarget = 1;
        m_AnimationTree = m_CurrentlyRenderedTree;
        m_CurrentlyRenderedTree = advancedDropdownItem;
    }
